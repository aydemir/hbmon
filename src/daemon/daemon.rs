...
use super::handler::{dispatch, Shared};
use super::pidfile;
...
impl Poll {
    fn new(
        ...
        // a previous run's bytes. Caller samples the length BEFORE spawning the
        // child (TASK-021) — sampling after spawn would misclassify the child's
        // first bytes as stale under parallel load.
        Self {
...
pub fn run_daemon(cfg: MonitorConfig) -> Result<(), String> {
    ...
    // Dep tarama sınırı spawn ÖNCESİ yakalanır (TASK-021): .out append-açık
    // olduğundan örnekleme spawn sonrasına kalırsa child'ın ilk baytları
    // (hızlı `echo dep...`) "stale prefix" sayılıp hiç taranmaz. Paralel
    // yükte daemon gecikince bu yarış kaybedilir; sınır spawn öncesi
    // uzunluktur — child'ın yazdığı her şey bunun sonrasındadır.
    let dep_offset = std::fs::metadata(&cfg.out).map(|m| m.len()).unwrap_or(0);
    ...
    *shared.last_event.lock().unwrap() = ready.clone();
    ...
    let mut st = Poll::new(cfg.timeout_sec, dep_offset, &cg);
